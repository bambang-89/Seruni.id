# Sistem Surat Desa (TTE + QR)

## 1. Ringkasan Sistem Surat (On Point)

- **Tujuan:** Warga urus surat keterangan tanpa datang ke kantor; dokumen sah diterima digital.
- **Kanal:** Web responsif + WhatsApp (1 nomor resmi terverifikasi).
- **Inti:** Pilih jenis surat → auto-fill identitas by NIK → isi DNA (field manual) → verifikasi → TTE Kades → QR + hash → kirim ke WA.
- **Status:** `DIAJUKAN → DIVERIFIKASI → DITANDATANGANI → DIKIRIM → ARSIP` (atau `DITOLAK` + alasan).
- **Keamanan:** Hash SHA-256 + QR verifikasi publik (`/verifikasi/{id_surat}`); scan QR → halaman validasi berisi **VCard pejabat penandatangan** (Kades/Sekdes/Kaur/Kasi) + **identitas surat** (rincian Tabel Arsip Surat khusus surat tersebut).
- **Pengarsipan:** `id_surat` (UUID) = Primary Key pengganti Nomor Urut; **Nomor Urut di-generate per jenis surat** (bukan keseluruhan) → arsip rapi & query cepat.
- **Efek ke Penduduk:** Surat tertentu (Kematian, Kelahiran, Pindah, dll.) otomatis memicu update `penduduk`/`log_penduduk` via event.
- **Jenis surat:** ~85 (dari konsolidasi `surat.csv` + `surat2.csv`), 100% online, tanpa duplikat, sesuai kewenangan desa.
- **Data:** `surat_jenis` (CRUD admin) → `surat_pengajuan` (`id_surat` PK) → `surat_dokumen` + `surat_log_status` (append-only) + `surat_arsip` (per jenis).

## 2. Workflow Lengkap Pelayanan Surat Online

### A. Alur Umum (Web & WhatsApp)

```
[Warga] 1. Pilih jenis surat
   │
   ▼
2. Auto-fill identitas (NIK → penduduk): Nama, TTL, JK, Agama,
   Status Kawin, Pekerjaan, Alamat, No. KK  ── (tidak diketik manual)
   │
   ▼
3. Isi DNA (field manual sesuai jenis surat)
   │
   ▼
4. Submit ──► status DIAJUKAN ──► event: surat.diajukan
   │
   ▼
[Admin/Verifikator] 5. Verifikasi data + lampiran (jika ada)
   ├── TOLAK ──► status DITOLAK (+ alasan) ──► WA ke warga ──► SELESAI
   │
   └── TERIMA ──► status DIVERIFIKASI ──► event: surat.diverifikasi
                      │
                      ▼
[Kepala Desa] 6. TTE (Tanda Tangan Elektronik)
   │
   ▼
7. status DITANDATANGANI ──► event: surat.diterbitkan
   │   • Nomor surat auto-generate (format klasifikasi arsip)
   │   • Generate QR + document_hash
   │
   ▼
8. Kirim dokumen (PDF ber-QR) ke WA warga ──► status DIKIRIM
   │
   ▼
9. ARSIP (masuk log status + rekap IDM)
```

### B. Kanal WhatsApp (Chatbot Rule-based)

| Tier          | Fungsi                | OTP   | Jam Proses        |
| ------------- | --------------------- | ----- | ----------------- |
| `info_instan` | Cek status/info surat | Tidak | 24 jam (langsung) |
| `transaksi`   | Ajukan surat          | Wajib | Hanya jam kerja   |

Alur `transaksi`: Menu jenis surat → tanya data pelengkap → autofill NIK → konfirmasi → OTP → submit → ikuti state machine di atas. Session per HP (`wa_chat_session`), expire jika idle.

### C. Aturan Kritikal

- **Nomor surat** digenerate otomatis (`KODE/NOMOR-URUT/BULAN-ROMAWI/TAHUN`), **tidak boleh input manual** (cegah duplikasi/nomor loncat). `NOMOR-URUT` **dihitung per jenis surat** (sequence per `jenis_surat_id` + tahun), bukan global.
- **`id_surat`** (UUID) adalah **Primary Key** pengganti Nomor Urut — rujukan internal & QR (`/verifikasi/{id_surat}`); Nomor Urut hanya label arsip, bisa berubah format tanpa memutus relasi FK ke modul lain.
- **QR code** hanya digenerate **setelah TTE sah** — tidak ada QR untuk DRAFT/DIAJUKAN.
- Field identitas (`auto`) selalu dari `penduduk`; pemohon hanya isi field `manual` (DNA).
- Lampiran bukti (ijazah, akta, SPPT, dll.) diunggah digital pada tahap verifikasi.

### D. Event & Integrasi

- `surat.diterbitkan` → **IDM Dimensi 6** (indikator 45.b, pemanfaatan teknnologi pelayanan): persentase surat digital vs manual dihitung ulang.
- `surat.diajukan` / `surat.diverifikasi` → notifikasi ke admin & Kades (antrean TTE).
- Semua perubahan status → `surat_log_status` (append-only, audit trail).

### E. Sistem QRCode & Halaman Validasi

QR code pada surat sah berisi `id_surat` (UUID) → saat di-scan buka `/verifikasi/{id_surat}` menampilkan:

**1. VCard Pejabat Penandatangan** (dari `desa_pamong` via `surat_dokumen.ttd_oleh_penduduk_id`):

- Nama, Jabatan (Kepala Desa / Sekretaris Desa / Kaur / Kasi), NIP (ops), foto/logo desa, status TTE sah.

**2. Identitas Surat — Tabel Arsip Surat (khusus surat tersebut):**

| Field            | Isi                                   |
| ---------------- | ------------------------------------- |
| `id_surat`       | UUID (Primary Key)                    |
| Nomor Surat      | `KODE/NOMOR-URUT/BULAN-ROMAWI/TAHUN`  |
| Kode Klasifikasi | mis. 474 / 477                        |
| Jenis Surat      | mis. Surat Keterangan Kematian        |
| Pemohon          | Nama (NIK disamarkan: `3201xxxx1234`) |
| Tgl Terbit       | tanggal TTE                           |
| Pejabat TT       | Nama + Jabatan                        |
| Status           | Sah / Tidak Ditemukan / Dicabut       |
| Hash Dokumen     | SHA-256 (verifikasi integritas)       |

> Privasi: hanya metadata non-sensitif; **tidak** menampilkan isi lengkap surat atau NIK penuh pemohon.

### F. Sistem Pengarsipan Surat

Dua kondisi wajib:

**a. Nomor Urut per jenis surat** — sequence dihitung per `jenis_surat_id` + tahun berjalan (bukan keseluruhan desa).

- Surat Domisili → urut `001..n` sendiri; Surat Usaha → urut `001..n` sendiri; dst. → arsip per jenis rapi & query cepat.

**b. `id_surat` sebagai Primary Key** — menggantikan Nomor Urut sebagai rujukan internal.

- `id_surat` (UUID) = PK `surat_pengajuan`; Nomor Urut hanya label arsip (bisa diubah format tanpa memutus FK ke modul lain: TTE, arsip, validasi, penduduk).

**Tabel `surat_arsip` (per jenis):**

| Kolom              | Keterangan                 |
| ------------------ | -------------------------- |
| `id_surat` (PK/FK) | → `surat_pengajuan.id`     |
| `jenis_surat_id`   | → `surat_jenis.id`         |
| `nomor_urut`       | sequence per jenis + tahun |
| `nomor_surat`      | format klasifikasi arsip   |
| `tahun`            | tahun berjalan             |
| `kode_klasifikasi` | parent code                |
| `tanggal_arsip`    | tgl TTE                    |
| `ttd_oleh`         | pejabat penandatangan      |
| `file_path`        | PDF arsip                  |
| `hash`             | SHA-256                    |

### G. Surat yang Mempengaruhi Data Penduduk

Saat TTE selesai, `surat.diterbitkan` + `kode_peristiwa` memicu worker update `penduduk`/`log_penduduk` (sama dengan mutasi di Sistem_Penduduk):

| Jenis Surat           | Kode  | Efek ke `penduduk`                      |
| --------------------- | ----- | --------------------------------------- |
| Keterangan Kematian   | 477.4 | `status_dasar` = MATI + tgl & sebab     |
| Keterangan Kelahiran  | 477.3 | INSERT bayi (HIDUP) + link KK ortu      |
| Belum Ada Akta Lahir  | 477.5 | INSERT bayi (HIDUP) + link KK ortu      |
| Pindah Domisili       | 475.0 | `status_dasar` = PINDAH + alamat tujuan |
| Mutasi Penduduk Masuk | 475.3 | INSERT/aktifkan + link KK & cluster     |
| Tidak Berada di Desa  | 475.5 | `status_dasar` = PERGI                  |
| KK Sementara          | 474.2 | INSERT `keluarga` baru                  |
| Ahli Waris            | 474.9 | update `status_kawin` ahli waris        |
| Status Janda/Duda     | 477.1 | `status_kawin` = JANDA/DUDA             |

> Aturan: update penduduk **wajib verifikasi admin** (sama dengan mutasi manual); surat hanya memicu draft perubahan, bukan auto-overwrite tanpa review.

## 3. Tabel Jenis Surat

> Sumber: `JENIS_SURAT_KONSOLIDASI.md`. Kolom: **Kode Klasifikasi | Kode Surat | Jenis Surat | DNA (field manual)**.
