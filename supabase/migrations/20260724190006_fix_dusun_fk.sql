-- Migration: 20260724190006_fix_wilayah_fk.sql
-- Idempotent: Clean orphan FK values from ALL wilayah columns in penduduk, then add FK constraints.
-- Safe to run multiple times. Run AFTER 20260724190005.
DO $outer$
DECLARE
  _cnt INT;
BEGIN
  ...
  -- ============================================================
  -- Create all ref tables if not exist (idempotent)
  -- ============================================================
  CREATE TABLE IF NOT EXISTS public.ref_provinsi (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(), kode VARCHAR(2) NOT NULL UNIQUE,
    nama VARCHAR(100) NOT NULL, ibukota VARCHAR(100), urutan INT NOT NULL DEFAULT 0,
  ...
  CREATE TABLE IF NOT EXISTS public.ref_kabupaten (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(), kode VARCHAR(4) NOT NULL UNIQUE,
    kode_provinsi VARCHAR(2) NOT NULL, nama VARCHAR(100) NOT NULL, ibukota VARCHAR(100),
    jenis VARCHAR(20) NOT NULL DEFAULT 'Kabupaten' CHECK (jenis IN ('Kabupaten', 'Kota')),
    urutan INT NOT NULL DEFAULT 0, aktif BOOLEAN NOT NULL DEFAULT true,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
  );
  ...
  CREATE TABLE IF NOT EXISTS public.ref_kecamatan (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(), kode VARCHAR(6) NOT NULL UNIQUE,
    kode_kabupaten VARCHAR(4) NOT NULL, nama VARCHAR(100) NOT NULL,
    urutan INT NOT NULL DEFAULT 0, aktif BOOLEAN NOT NULL DEFAULT true,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
  );
  ...
  CREATE TABLE IF NOT EXISTS public.ref_desa (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(), kode VARCHAR(10) NOT NULL UNIQUE,
    kode_kecamatan VARCHAR(6) NOT NULL, nama VARCHAR(100) NOT NULL,
    jenis VARCHAR(20) NOT NULL DEFAULT 'Desa' CHECK (jenis IN ('Desa', 'Kelurahan')),
    kode_pos VARCHAR(10), urutan INT NOT NULL DEFAULT 0, aktif BOOLEAN NOT NULL DEFAULT true,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
  );
  ...
  CREATE TABLE IF NOT EXISTS public.ref_rw (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(), kode VARCHAR(20) NOT NULL,
    kode_desa VARCHAR(10) NOT NULL, rw VARCHAR(5) NOT NULL,
    aktif BOOLEAN NOT NULL DEFAULT true,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    UNIQUE(kode_desa, rw)
  );
  ...
  CREATE TABLE IF NOT EXISTS public.ref_rt (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(), kode VARCHAR(20) NOT NULL,
    kode_rw VARCHAR(20) NOT NULL, rt VARCHAR(5) NOT NULL,
    aktif BOOLEAN NOT NULL DEFAULT true,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    UNIQUE(kode_rw, rt)
  );
  ...
  CREATE TABLE IF NOT EXISTS public.ref_dusun (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(), kode VARCHAR(10) NOT NULL UNIQUE,
    nama VARCHAR(100) NOT NULL, urutan INT NOT NULL DEFAULT 0, aktif BOOLEAN NOT NULL DEFAULT true,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
  );
  ...
  RAISE NOTICE 'All ref tables ready.';

  -- ============================================================
  -- Add FK columns to penduduk if not exist
  -- ============================================================
  ...
  -- ============================================================
  -- Step 3: Seed ref_kecamatan (Lombok Timur)
  -- ============================================================
  ...
  -- ============================================================
  -- Step 4: Seed ref_desa (Pringgabaya)
  -- ============================================================
  ...
  -- ============================================================
  -- Step 5: Seed ref_dusun from penduduk.dusun + wilayah_dusun
  -- ============================================================
  INSERT INTO public.ref_dusun (kode, nama, urutan, aktif)
  SELECT DISTINCT
    'DSN-' || LPAD(ROW_NUMBER() OVER (ORDER BY dusun)::TEXT, 3, '0'),
    TRIM(dusun),
    1,
    true
  FROM (
    SELECT DISTINCT p.dusun FROM public.penduduk p
    WHERE p.dusun IS NOT NULL AND TRIM(p.dusun) <> '' AND LENGTH(TRIM(p.dusun)) >= 2
    UNION
    SELECT DISTINCT wd.nama FROM public.wilayah_dusun wd
    WHERE wd.nama IS NOT NULL AND TRIM(wd.nama) <> ''
  ) combined
  WHERE NOT EXISTS (SELECT 1 FROM public.ref_dusun rd WHERE rd.nama = TRIM(combined.dusun))
  ON CONFLICT (kode) DO NOTHING;

  -- ============================================================
  -- Step 6: Backfill ALL FK columns
  -- ============================================================
  ...
  WHERE p.dusun IS NOT NULL AND TRIM(p.dusun) <> ''
    AND LOWER(p.dusun) = LOWER(d.nama)
    AND p.dusun_id IS NULL;

  -- Backfill provinsi/kabupaten/kecamatan/desa (static — Seruni Mumbul)
  UPDATE public.penduduk
  SET
    provinsi_id = COALESCE(provinsi_id, (SELECT id FROM public.ref_provinsi WHERE kode = '52' LIMIT 1)),
    kabupaten_id = COALESCE(kabupaten_id, (SELECT id FROM public.ref_kabupaten WHERE kode = '5204' LIMIT 1)),
    kecamatan_id = COALESCE(kecamatan_id, (SELECT id FROM public.ref_kecamatan WHERE kode = '520401' LIMIT 1)),
    desa_id = COALESCE(desa_id, (SELECT id FROM public.ref_desa WHERE kode = '5204011001' LIMIT 1))
  WHERE tenant_id IS NOT NULL
    AND (provinsi_id IS NULL OR kabupaten_id IS NULL OR kecamatan_id IS NULL OR desa_id IS NULL);

  -- ============================================================
  -- Step 7: Clear orphan FK values
  -- ============================================================
  RAISE NOTICE 'Clearing orphan FK values...';

  UPDATE public.penduduk SET dusun_id = NULL WHERE dusun_id IS NOT NULL
    AND dusun_id NOT IN (SELECT id FROM public.ref_dusun);
  GET DIAGNOSTICS _cnt = ROW_COUNT;
  RAISE NOTICE '  Cleared % orphan dusun_id', _cnt;

  UPDATE public.penduduk SET rw_id = NULL WHERE rw_id IS NOT NULL
    AND rw_id NOT IN (SELECT id FROM public.ref_rw);
  GET DIAGNOSTICS _cnt = ROW_COUNT;
  RAISE NOTICE '  Cleared % orphan rw_id', _cnt;

  UPDATE public.penduduk SET rt_id = NULL WHERE rt_id IS NOT NULL
    AND rt_id NOT IN (SELECT id FROM public.ref_rt);
  GET DIAGNOSTICS _cnt = ROW_COUNT;
  RAISE NOTICE '  Cleared % orphan rt_id', _cnt;

  -- ============================================================
  -- Step 8: Drop existing FK constraints if exist
  -- ============================================================
  ALTER TABLE public.penduduk DROP CONSTRAINT IF EXISTS fk_penduduk_provinsi;
  ALTER TABLE public.penduduk DROP CONSTRAINT IF EXISTS fk_penduduk_kabupaten;
  ALTER TABLE public.penduduk DROP CONSTRAINT IF EXISTS fk_penduduk_kecamatan;
  ALTER TABLE public.penduduk DROP CONSTRAINT IF EXISTS fk_penduduk_desa;
  ALTER TABLE public.penduduk DROP CONSTRAINT IF EXISTS fk_penduduk_dusun;
  ALTER TABLE public.penduduk DROP CONSTRAINT IF EXISTS fk_penduduk_rw;
  ALTER TABLE public.penduduk DROP CONSTRAINT IF EXISTS fk_penduduk_rt;

  -- ============================================================
  -- Step 9: Add FK constraints (all data now valid)
  -- ============================================================
  ...
  RAISE NOTICE 'All FK constraints added successfully.';
