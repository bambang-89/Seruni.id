-- ============================================================
-- MIGRASI: 20260834000002_update_find_penduduk_by_nik.sql
-- Deskripsi: Update RPC function untuk lookup NIK dengan join tabel referensi
-- ============================================================
...
BEGIN
  ...
  LEFT JOIN public.wilayah_provinsi rprov ON rprov.id = p.provinsi_id
  LEFT JOIN public.wilayah_kabupaten rkab ON rkab.id = p.kabupaten_id
  LEFT JOIN public.wilayah_kecamatan rkec ON rkec.id = p.kecamatan_id
  LEFT JOIN public.wilayah_desa rdesa ON rdesa.id = p.desa_id
  LEFT JOIN public.wilolos_batas rd ON rd.id = p.dusun_id
  WHERE p.nik = p_nik
  LIMIT 1;
END;
$$;
