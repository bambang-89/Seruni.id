 ...
 * Run: node scripts/seed-db-direct.mjs
 */
...
async function seed() {
  ...
  try {
    ...
    await client.query('DELETE FROM wilayah_dusun');
    await client.query(`
    ...
    await client.query(`
      ...
      ('posyandu-sasak', 'Posyandu', 'Posyandu Balita Sasak', '2026-08-15', '08.00–11.00 WITA', 'Posyandu Mawar II', 'PKK & Puskesmas', 'Pemeriksaan kesehatan rutin.'),
      ('verifikasi-dtks', 'Verifikasi', 'Verifikasi DTKS Semester II', '2026-08-20', '09.00–14.00 WITA', 'Seluruh Burnett', 'Kasi Kesra', 'Pemutakhiran data kesejahteraan.')
    `);
    console.log('✅ agenda: 7 agenda');
    ...
    await client.query(`
      ...
      ('Turnamen Bola Burnett', 'bola', 'Olahraga', '2026-05-30', 6),
    ...
    await client.query(`
      ...
      ('146/PMR/SM/VII/2026', '2026-07-10', 'Pemadaman Air Bersih Burnett', 'Perbaikan pipa PAMDes.'),
      ('142/PMR/SM/VII/2026', '2026-07-04', 'Pendaftaran Beasiswa Burnett Nelayan', '5–20 Juli 2026.'),
      ('138/PMR/SM/VI/2026', '2026-06-28', 'Verifikasi Ulang DTKS Semester II', 'Kader dusun berkunjung.')
    `);
    ...
    await client.query('DELETE FROM potensi_wisata');
    await client.query(`
      ...
      ($1, 'Bukit Panorama Timba Gading', 'Ekowisata', 'Sasak', 'Titik pandang matahari terbit di 380 mdpl.', 'Jalur tracking', -8.5210, 116.6780, 'publish'),
      ($1, 'Sentra Tenun Songket Sasak', 'Wisata Budaya', 'Mandar', 'Sanggar tenun aktif. Pengunjung dapat mencoba.', 'Sanggar tenun', -8.5350, 116.6620, 'publish'),
      ($1, 'Taman Buru Klasahan', 'Wisata Alam', 'Mandar', 'Hutan klasahan untuk tracking.', 'Pos jaga', -8.5580, 116.6450, 'publish')
    `, [tenantId]);
    console.log('✅ potensi_wisata: 4 destinasi');

    // ============================================================
    // 7. POTENSI UMKM
    // ============================================================
    console.log('\n🏪 Seeding potensi_umkm...');
    await client.query('DELETE FROM potensi_umkm');
    await client.query(`
      INSERT INTO potensi_umkm (tenant_id, tipe, nama, pemilik, sektor, dusun, kontak, alamat, deskripsi, status) VALUES
      ($1, 'Kuliner', 'UMKM Madu Trigona Seruni', 'Hj. Rina', 'Perlebahan', 'Mandar', '+6281234567101', 'Mandar', 'Madu trigona premium.', 'publish'),
      ($1, 'Kuliner', 'Koperasi Tani Maju', 'Andi Rahman', 'Pertanian', 'Sasak', '+6281234567102', 'Sasak', 'Kopi robusta pilihan.', 'publish'),
      ($1, 'Kerajinan', 'Sanggar Tenun Ibu Aminah', 'Siti Aminah', 'Kerajinan', 'Mandar', '+6281234567103', 'Mandar', 'Tenun songket Sasak.', 'publish'),
      ($1, 'Kuliner', 'UMKM Brangtapen Asri Mumbul', 'H. Basri', 'Perikanan', 'Brangtapen Asri', '+6281234567104', 'Brangtapen Asri', 'Olahan rumput laut.', 'publish'),
      ($1, 'Kuliner', 'Warung Seruni', 'Hj. Maryam', 'Kuliner', 'Pusat Desa', '+6281234567105', 'Pusat Desa', 'Makanan khas Lombok.', 'publish'),
      ($1, 'Kerajinan', 'Pengrajin Bambu Sasak', 'Lahudin', 'Kerajinan', 'Sasak', '+6281234567106', 'Sasak', 'Anyaman bamboe.', 'publish')
    `, [tenantId]);
    console.log('✅ potensi_umkm: 6 UMKM');

    // ============================================================
    // 8. POTENSI PRODUK
    // ============================================================
    console.log('\n🛒 Seeding potensi_produk...');
    await client.query('DELETE FROM potensi_produk');
    await client.query(`
      INSERT INTO potensi_produk (tenant_id, penjual_nama, nama, kategori, harga, satuan, stok, deskripsi, featured, status) VALUES
      ($1, 'Hj. Rina', 'Madu Trigona Seruni 500ml', 'Makanan', 95000, 'botol', 50, 'Madu trigona premium.', true, 'publish'),
      ($1, 'Andi Rahman', 'Kopi Robusta Sembalun 250g', 'Minuman', 65000, '250g', 100, 'Kopi robusta pilihan.', true, 'publish'),
      ($1, 'Siti Aminah', 'Tenun Songket Sasak Motif Seruni', 'Kerajinan', 450000, 'helai', 15, 'Tenun songket asli.', true, 'publish'),
      ($1, 'H. Basri', 'Kerupuk Rumput Laut 200g', 'Makanan', 22000, 'bungkus', 200, 'Kerupuk rumput laut.', true, 'publish'),
      ($1, 'Siti Aminah', 'Sapu Nyiru Anyaman', 'Kerajinan', 75000, 'pcs', 30, 'Sapu nyiru dari bamboo.', true, 'publish'),
      ($1, 'Hj. Rina', 'Propoliss Trigona 100ml', 'Obat Tradisional', 120000, 'botol', 25, 'Propoliss trigona.', false, 'publish')
    `, [tenantId]);
    console.log('✅ potensi_produk: 6 produk');

    // ============================================================
    // 9. KEGIATAN PEMBANGUNAN
    // ============================================================
    console.log('\n🏗️ Seeding kegiatan_pembangunan...');
    await client.query('DELETE FROM kegiatan_pembangunan');
    await client.query(`
      ...
      ($1, 2026, 'Pembangunan Desa', 'Rehabilitasi Saluran Irigasi Mandar', 'Mandar', '1.2 km', 280000000, 229600000, 'APBDes', 'diproses'),
      ($1, 2026, 'Pembangunan Desa', 'Pembangunan MCK Umum Pasar Seruni', 'Pusat Desa', '1 unit', 150000000, 67500000, 'APBDes', 'diproses'),
      ($1, 2026, 'Pembangunan Desa', 'Pengadaan Lampu PJU Surya', 'Seluruh Desa', '30 titik', 90000000, 27000000, 'APBDes', 'diproses'),
      ($1, 2026, 'Pembangunan Desa', 'Pengerasan Jalan Poros Burnett Mandar', 'Mandar', '0.8 km', 320000000, 249600000, 'APBDes', 'diverifikasi'),
      ($1, 2026, 'Pembangunan Desa', 'Pemasangan Bronjong Pantai Seruni', 'Brangtapen Asri', '200 meter', 180000000, 180000000, 'APBDes', 'selesai'),
      ($1, 2025, 'Pembangunan Desa', 'Normalisasi Sungai Brangtapen Asri', 'Brangtapen Asri', '500 meter', 200000000, 195000000, 'APBDes', 'selesai')
    `, [tenantId]);
    console.log('✅ kegiatan_pembangunan: 6 kegiatan');

    // ============================================================
    // 10. USULAN WARGA
    // ============================================================
    console.log('\n📝 Seeding usulan_warga...');
    await client.query('DELETE FROM usulan_warga');
    await client.query(`
      ...
      ($1, 'USL-2026-001', 'Ahmad Zulkifli', '+6281234567001', 'Mandar', 'infrastruktur', 'Perbaikan Jalan Poros Mandar-Sasak', 'Jalan rusak 2,3 km.', 'Mandar-Sasak', 'ditindaklanjuti', 342),
      ($1, 'USL-2026-002', 'Siti Aminah', '+6281234567002', 'Mandar', 'pendidikan', 'Pembangunan PAUD Terpadu Mandar', 'PAUD untuk 87 balita.', 'Mandar', 'ditindaklanjuti', 289),
      ($1, 'USL-2026-003', 'Muhammad Ali', '+6281234567003', 'Brangtapen Asri', 'infrastruktur', 'Sumur Bor Air Bersih Burnett', 'Air bersih terbatas.', 'Brangtapen Asri', 'diverifikasi', 251),
      ($1, 'USL-2026-004', 'Hajjah Rahayu', '+6281234567004', 'Mandar', 'kesehatan', 'Renovasi Poskesdes Utama', 'Poskesdes perlu renovasi.', 'Pusat Desa', 'diverifikasi', 198),
      ($1, 'USL-2026-005', 'Budi Santoso', '+6281234567005', 'Brangtapen Asri', 'sosial', 'Beasiswa Burnett Nelayan Tidak Mampu', '12 anak nelayan.', 'Brangtapen Asri', 'ditindaklanjuti', 176),
      ($1, 'USL-2026-006', 'Rina Marlina', '+6281234567006', 'Mandar', 'ekonomi', 'Pelatihan Digital Marketing UMKM', 'Pemasaran digital.', 'Mandar', 'diverifikasi', 154),
      ($1, 'USL-2026-007', 'H. Lalu Husain', '+6281234567007', 'Brangtapen Asri', 'lingkungan', 'Pengadaan Kapal Sampah Pantai', 'Sampah plastik.', 'Brangtapen Asri', 'diverifikasi', 132),
      ($1, 'USL-2026-008', 'Nurhayati', '+6281234567008', 'Sasak', 'pendidikan', 'Rumah Baca Anak Sasak', 'Ruang baca anak.', 'Sasak', 'diverifikasi', 118),
      ($1, 'USL-2026-009', 'Ahmad Fauzi', '+6281234567009', 'Mandar', 'sosial', 'Rehabilitasi Lapangan Sepakbola', 'Lapangan perlu rehab.', 'Mandar', 'diverifikasi', 97),
      ($1, 'USL-2026-010', 'Siti Zahra', '+6281234567010', 'Brangtapen Asri', 'lingkungan', 'Bank Sampah Terpadu', 'Sampah terpadu.', 'Brangtapen Asri', 'diverifikasi', 84),
      ($1, 'USL-2026-011', 'H. Hasan', '+6281234567011', 'Sasak', 'infrastruktur', 'Perbaikan Jembatan Sasak', 'Jembatan perlu perbaikan.', 'Sasak', 'diverifikasi', 76),
      ($1, 'USL-2026-012', 'Mariam', '+6281234567012', 'Mandar', 'kesehatan', 'Pos Kesehatan Gigi', 'Dokter gigi di Poskesdes.', 'Mandar', 'diverifikasi', 65)
    `, [tenantId]);
    console.log('✅ usulan_warga: 12 usulan');
    ...
    // ============================================================
    console.log('\n👥 Seeding desa_pamong...');
    await client.query('DELETE FROM desa_pamong');
    await client.query(`
      ...
      ($1, 'Ahmad Fauzi', 'Kaur Perencanaan', NULL, 7),
      ($1, 'Lalu Ismail', 'Kaur Tata Usaha dan Umum', NULL, 8)
    `, [tenantId]);
    console.log('✅ desa_pamong: 8 perangkat');

    // ============================================================
    // 13. LEMBAGA DESA
    // ============================================================
    console.log('\n🏛️ Seeding lembaga_desa...');
    await client.query('DELETE FROM lembaga_desa');
    await client.query(`
      ...
      ($1, 'Karang Taruna Seruni', 'Ahmad Rizki', 42, 4),
      ($1, 'Linmas', 'Muhammad Yusuf', 18, 5),
      ($1, 'BUMDes Bina Seruni Mandiri', 'Baiq Salma', 7, 6)
    `, [tenantId]);
    console.log('✅ lembaga_desa: 6 lembaga');
    ...
    const result = await client.query(`
      SELECT 'wilayah_dusun' as tbl, count(*)::text as jml FROM wilayah_dusun
