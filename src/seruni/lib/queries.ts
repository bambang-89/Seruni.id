...
export function useAgendaById(id?: string) {
  ...
  useEffect(() => {
    ...
    supabase.from("agenda").select("*").eq("id", id).maybeSingle()
      .then(({ data: r }) => { setData((r as Agenda | null) || null); setLoading(false); });
  }, [id]);
  return { data, loading };
}
...
export function useGaleriById(id?: string) {
  ...
  useEffect(() => {
    ...
    supabase.from("galeri").select("*").eq("id", id).maybeSingle()
      .then(({ data: r }) => { setData((r as Galeri | null) || null); setLoading(false); });
  }, [id]);
  return { data, loading };
}
...
export function usePengumumanById(id?: string) {
  ...
  useEffect(() => {
    ...
    supabase.from("pengumuman").select("*").eq("id", id).maybeSingle()
      .then(({ data: r }) => { setData((r as Pengumuman | null) || null); setLoading(false); });
  }, [id]);
  return { data, loading };
}
...
export function usePosyanduById(id?: string) {
  ...
  useEffect(() => {
    ...
    supabase.from("posyandu_agregat").select("*").eq("id", id).maybeSingle()
      .then(({ data: r }) => { setData((r as PosyanduAgregat | null) || null); setLoading(false); });
  }, [id]);
  return { data, loading };
}
...
export function useStuntingById(id?: string) {
  ...
  useEffect(() => {
    ...
    supabase.from("stunting_agregat").select("*").eq("id", id).maybeSingle()
      .then(({ data: r }) => { setData((r as StuntingAgregat | null) || null); setLoading(false); });
  }, [id]);
  return { data, loading };
}
...
export function useUmkmById(id?: string) {
  ...
  useEffect(() => {
    ...
    supabase.from("potensi_umkm").select("*").eq("id", id).maybeSingle()
      .then(({ data: r }) => { setData((r as PotensiUmkm | null) || null); setLoading(false); });
  }, [id]);
  return { data, loading };
}
  ...
  useEffect(() => {
    ...
    supabase.from("potensi_produk").select("*").eq("id", id).maybeSingle()
      .then(({ data: r }) => {
        ...
        setData(raw ? ({ ...raw, harga: raw.harga == null ? null : Number(raw.harga) } as unknown as PotensiProduk) : null);
        setLoading(false);
      });
  }, [id]);
  return { data, loading };
  ...
  useEffect(() => {
    ...
    supabase.from("potensi_wisata").select("*").eq("id", id).maybeSingle()
      .then(({ data: r }) => {
        ...
        } as unknown as PotensiWisata) : null);
        setLoading(false);
      });
  }, [id]);
  return { data, loading };
...
export type KegiatanPembangunan = {
  id: string; tahun: number | null; bidang: string | null; judul: string | null;
  nama_kegiatan: string | null; lokasi: string | null; dusun: string | null;
  volume: string | null; anggaran: number | null; sumber_dana: string | null;
  status: string | null; tanggal_mulai: string | null; tanggal_selesai: string | null;
  keterangan: string | null; gambar_url: string | null; gambar_dokumentasi: unknown | null;
  progress_persen: number | null; created_at: string;
};

export function usePembangunanById(id?: string) {
  const [data, setData] = useState<KegiatanPembangunan | null>(null);
  const [loading, setLoading] = useState(true);
  useEffect(() => {
    if (!id) { setLoading(false); return; }
    supabase.from("kegiatan_pembangunan").select("*").eq("id", id).maybeSingle()
      .then(({ data: r }) => {
        ...
        setData(raw ? ({
          ...raw,
          tahun: raw.tahun == null ? null : Number(raw.tahun),
          anggaran: raw.anggaran == null ? null : Number(raw.anggaran),
          progress_persen: raw.progress_persen == null ? null : Number(raw.progress_persen),
        } as unknown as KegiatanPembangunan) : null);
        setLoading(false);
      });
  }, [id]);
  return { data, loading };
...
export function useBansosById(id?: string) {
  ...
  useEffect(() => {
    ...
    supabase.from("bantuan_sosial").select("*").eq("id", id).maybeSingle()
      .then(({ data: r }) => { setData((r as BantuanSosial | null) || null); setLoading(false); });
  }, [id]);
  return { data, loading };
}

export type AduanRow = {
  id: string; nomor_tiket: string; nama_pelapor: string; kontak: string;
  kategori: string; judul: string; isi: string; lokasi: string | null;
  lampiran_url: string | null; status: string; tanggapan: string | null;
  ditanggapi_pada: string | null; created_at: string;
};

export function useAduanById(id?: string) {
  const [data, setData] = useState<AduanRow | null>(null);
  const [loading, setLoading] = useState(true);
  useEffect(() => {
    if (!id) { setLoading(false); return; }
    supabase.from("aduan_warga").select("*").eq("id", id).maybeSingle()
      .then(({ data: r }) => { setData((r as AduanRow | null) || null); setLoading(false); });
  }, [id]);
  return { data, loading };
}

export type IdmIndikatorRow = {
  id: string; tahun: number | null; dimensi: string | null; indikator: string | null;
  nilai: number | null; skor: number | null; sumber: string | null;
  keterangan: string | null; published: boolean;
};

export function useIdmIndikatorById(id?: string) {
  const [data, setData] = useState<IdmIndikatorRow | null>(null);
  const [loading, setLoading] = useState(true);
  useEffect(() => {
    if (!id) { setLoading(false); return; }
    supabase.from("idm_indikator").select("*").eq("id", id).maybeSingle()
      .then(({ data: r }) => {
        ...
        setData(raw ? ({
          ...raw,
          tahun: raw.tahun == null ? null : Number(raw.tahun),
          nilai: raw.nilai == null ? null : Number(raw.nilai),
          skor: raw.skor == null ? null : Number(raw.skor),
        } as unknown as IdmIndikatorRow) : null);
        setLoading(false);
      });
  }, [id]);
  return { data, loading };
